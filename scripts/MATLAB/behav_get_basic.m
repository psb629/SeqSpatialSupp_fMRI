function R = behav_get_basic(behav_data, glm)
%%%%%%%%%%%%%%%%%%%%%
%%%     Input     %%%
%%%%%%%%%%%%%%%%%%%%%
%   behav_data: str
...
nrepID = find(~ismember([1:nTrials], [repcueID repmotorID nintID]));
...
%    mov = movload(fullfile(workdir, behavDir, sprintf(data_format, s, s, r)));
    idx = find(S.BN==r);
    tS(r,S.cueP(idx) == seqences(1) & S.seqType(idx)==0) = 0;
    tS(r,S.cueP(idx) == seqences(1) & S.seqType(idx)==1) = 1;
    tS(r,S.cueP(idx) == seqences(2) & S.seqType(idx)==0) = 2;
    tS(r,S.cueP(idx) == seqences(2) & S.seqType(idx)==1) = 3;
    tS(r,S.cueP(idx) == seqences(3) & S.seqType(idx)==0) = 4;
    tS(r,S.cueP(idx) == seqences(3) & S.seqType(idx)==1) = 5;
    tS(r,S.cueP(idx) == seqences(4) & S.seqType(idx)==0) = 6;
    tS(r,S.cueP(idx) == seqences(4) & S.seqType(idx)==1) = 7;
...
for r=1:nRuns
    ...
    R.isNint(r,:) = ~ismember([1:nTrials], find(R.isRepMotor(r,:)+R.isRepCue(r,:)+R.isNRep(r,:)~=0));

    R.MT(r,:) = S.MT(nTrials*(r-1)+1:nTrials*r);
    R.RT(r,:) = S.RT(nTrials*(r-1)+1:nTrials*r);
    
end
R.isLetter = zeros(nRuns, nTrials);
R.isSpatial = zeros(nRuns, nTrials);
...
elseif glm==1
    R.cond = repmat([1:nTrials],nRuns, 1);
