function [SPM] = spmj_spm_crossval(SPM,est_fcn)
% [Re]ML Estimation of a General Linear Model
% FORMAT [SPM] = spmj_spm_crossval(SPM)
% 
% Tests crossvalidation of the timeseries 
% 
...
%         xX.name   - C condition T type: for beta cross-validation  
%       - Optional fields are:
...
%                           ----------------
%
% RSS_0.nii                   - Total sums of square (accounted for block mean)
% RSS_1.nii                   - Predictive sums of square after accounting for
%                               noise regressors 
% RSS_2.nii                   - predictive sums of square 
% R2partial.nii               - predictive power of each voxel: RSS_2.nii/RSS_1.nii
% Bstats_???.nii              - Prediction accuracy on the betas 
%
%                           ----------------
% 
% Hyper_1.nii                    - averaged hyperparameter 
% Hyper_2.nii                    - averaged hyperparameter (last one is
%                                  noise variance)
%
%
%--------------------------------------------------------------------------
...
files = {'^mask\..{3}$','^ResMS\..{3}$','^RSS_.{1}\..{3}$','^Hyper_.{1}\..{3}$','^Bstats_.{1}\..{3}$','^R2partial_\..{3}$',...
    '^beta_.{4}\..{3}$','^beta_.{4}\..{3}$','^hrf_.{4}\..{3}$','^con_.{4}\..{3}$','^ResI_.{4}\..{3}$',...
...
%==========================================================================
for s=1:nRun
    xX.iiS=zeros(1,nBeta);   % number of the session
    xX.iiC=zeros(1,nBeta);   % number of the condition
    xX.iiT=zeros(1,nBeta);   % Number of trial type 
    xX.iiB=zeros(1,nBeta);  % number of the basis function
    xX.iiN=zeros(1,nBeta);   % Flag for the regressors of no interest (+ intercept = 2 )
end;
...
[nBstats,nHyper]=feval(est_fcn);     % Get number of B-stats images and number of hyperparameters  
xX=feval(est_fcn,xX,[],SPM.Sess);       % Get necessary precomputation on design matrix 
...
Vhyper(1:nHyper) = deal(struct(...
    ...
    'descrip',  ''));
for i = 1:nHyper
    Vhyper(i).fname   = sprintf('hyper_%04d.nii',i);
    Vhyper(i).descrip = sprintf('spm_spm:hyper (%02d) - ',i);
end
Vhyper = spm_create_vol(Vhyper);


%-Initialise R2partial image 
%----------------------------------------------------------------------
VR2partial = struct('fname',    'R2partial.nii',...
    'dim',      DIM',...
    'dt',       [spm_type('float64') spm_platform('bigend')],...
    'mat',      M,...
    'pinfo',    [1 0 0]',...
    'descrip',  'spm_spm:Residual sum-of-squares');
VR2partial = spm_create_vol(VR2partial);


%-Initialise predictive RSS images 
%----------------------------------------------------------------------
Vrss(1:3) = deal(struct(...
    'fname',    [],...
    'dim',      DIM',...
    'dt',       [spm_type('float32') spm_platform('bigend')],...
    'mat',      M,...
...
for i = 1:3
    Vrss(i).fname   = sprintf('RSS_%d.nii', i);
    Vrss(i).descrip = sprintf('spmj_spm_crossval:RSS');
end
Vrss = spm_create_vol(Vrss);
fprintf('%s%30s\n',repmat(sprintf('\b'),1,30),'...initialised');    %-#


%-Bstats images 
%----------------------------------------------------------------------
Vbstats(1:nBstats) = deal(struct(...
    'fname',    [],...
    'dim',      DIM',...
    'dt',       [spm_type('float32') spm_platform('bigend')],...
    'mat',      M,...
...
for i = 1:nBstats
    Vbstats(i).fname   = sprintf('bstats_%d.nii', i);
    Vbstats(i).descrip = sprintf('spmj_spm_crossval:bstats');
end
Vbstats = spm_create_vol(Vbstats);
fprintf('%s%30s\n',repmat(sprintf('\b'),1,30),'...initialised');    %-#
...
for z = 1:nbz:zdim                       %-loop over planes (2D or 3D data)
    ...
    CrHy = [];                           %-Hyperparameters 
    CrRSS   = [];                        % Residual sum or sqaure - crossvalidation 
    CrR2Partial = [];                    % Partial "R2" on crossvalidated 
    CrBstats = [];                       % Beta stats  
    ...
    for bch = 1:nbch                     %-loop over blocks
        ...
        if CrS
            ...
            [beta,RSS,Bstats,Hy]=feval(est_fcn,xX,KWY,SPM.Sess); 
            ...
            CrHy    = [CrHy, Hy];
            CrRSS      = [CrRSS, RSS];
            CrR2Partial = [CrR2Partial, (RSS(3,:)-RSS(2,:))./RSS(2,:)]; 
            CrBstats   = [CrBstats,Bstats]; 
    ...
    %-Plane complete, write plane to image files
    %======================================================================
    ...
    %------------------------------------------------------------------
    for i = 1:nHyper
        if ~isempty(Q), jj(Q) = CrHy(i,:);
        end
        Vhyper(i) = spm_write_plane(Vhyper(i), jj, CrPl);
    end
    
    %-Write RSS images
    %------------------------------------------------------------------
    for i = 1:3
        if ~isempty(Q), jj(Q) = CrRSS(i,:);
        end
        Vrss(i) = spm_write_plane(Vrss(i), jj, CrPl);
    end
    
    %-Write PartialR2
    %------------------------------------------------------------------
    if ~isempty(Q), jj(Q) = CrR2Partial; end
    VR2partial  = spm_write_plane(VR2partial, jj, CrPl);
    ...
    %------------------------------------------------------------------
    for i = 1:nBstats
        if ~isempty(Q), jj(Q) = CrBstats(i,:);
        end
        Vbstats(i) = spm_write_plane(Vbstats(i), jj, CrPl);
    end
...
SPM.Vhyper     = Vhyper;            %-Filehandle - Hyperparameter
SPM.Vrss       = Vrss;              % File handel RSS
SPM.Vbstats    = Vbstats;              % File handel RSS
