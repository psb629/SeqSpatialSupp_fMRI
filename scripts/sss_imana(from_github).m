...
% cd /srv/diedrichsen/data/SeqSpatialSupp_fMRI
% mkdir temp
% singularity run /srv/containers/cfmm2tar_v1.0.0.sif -p Diedrichsen^SSS -d
% 'yyyymmdd'. (Don't forget . )
% mkdir BIDS
% singularity run /srv/containers/tar2bids_v0.2.4.sif -o ./BIDS Diedrich*.tar
% edit sss_heuristic.py, change S01 -> SX
% singularity run /srv/containers/tar2bids_v0.2.4.sif -h SSS_heuristic.py -o ./BIDS <filename>.tar
% make sure that there are anat, fmap, func folders in ./BIDS/sub-SXX
% mkdir BIDS_gradcorrect
% singularity run /srv/containers/khanlab_gradcorrect_v0.0.3a.sif ./BIDS ./BIDS_gradcorrect participant --grad_coeff_file /srv/software/gradcorrect/coeff_AC84.grad
% Change folder names BIDS_gradcorrect -> BIDS and BIDS->BIDS_org, or move
% files in BIDS_gradcorrect to BIDS/sub-SXX and those in BIDS to BIDS_org/sub-SXX
...
switch(what)
    ...
    case 'BIDS:move_unzip_raw_anat'
        ...
        % anat_raw_path = fullfile(baseDir,BIDSDir,sprintf('sub-s%.02d',sn),'ses-01','anat',[char(pinfo.AnatRawName(pinfo.sn==sn)) '.nii.gz']);
    ...
    case 'BIDS:move_unzip_raw_func'
        ...
        for i = 1:length(run_list)
            ...
            func_raw_path = fullfile(baseDir,BIDSDir,sprintf('sub-S%.02d',sn),'func',FuncRawName_tmp);
    ...
    case 'BIDS:move_unzip_raw_fmap'
        ...
        for sess = 1:pinfo.numSess(pinfo.sn==sn)
            ...
            magnitude_path = fullfile(baseDir,BIDSDir,sprintf('sub-S%.02d',sn),'fmap',magnitude);
            phase_path = fullfile(baseDir,BIDSDir,sprintf('sub-S%.02d',sn),'fmap',phase);
    ...
    case 'FUNC:realign_unwarp'      
        ...
        subfolderFieldmap = '';
        spmj_realign_unwarp(baseDir,char(pinfo.subj_id(pinfo.sn==sn)),run_list_cell, startTR, inf, ...
                            'prefix',prefixepi,...
                            'rawdataDir',fullfile(baseDir,imagingRawDir,char(pinfo.subj_id(pinfo.sn==sn))),...
                            'subfolderFieldmap',subfolderFieldmap,...
                            'rtm',rtm);
...
%                     J.sess(r).cond(c).duration = R.dur(r,find(R.cond(r,:)==c));
        ...
        J.volt = 1;
        J.global = 'None';
        J.mask = {fullfile(baseDir,anatomicalDir, sprintf('S%02d', sn), 'rmask_noskull.nii,1')};
        J.mthresh = 0.05;
    ...
    case 'GLM:estimate' % estimate beta coefficient
        ...
        load(fullfile(baseDir,sprintf(glmDir, glm), sprintf('S%02d', sn), 'SPM.mat'));
        SPM.swd = fullfile(baseDir,sprintf(glmDir, glm), sprintf('S%02d', sn));
        % Run the GLM.
        if fig==0
            spm_rwls_spm(SPM);
        else
...
%                             'BothRep-L''BothRep-S','NonRep-L','NonRep-S','Non-Interest'};                
        ...
        save(fullfile(baseDir, sprintf(glmDir, glm), sprintf('S%02d', sn), 'SPM_light.mat'), 'SPM')
       % rename contrast images and spmT images
        conName = {'con','spmT'};
        for i = 1:length(SPM.xCon)
            for n=1
%             for n = 1:numel(conName)
                oldName = fullfile(baseDir, sprintf(glmDir, glm), sprintf('S%02d', sn), sprintf('%s_%2.4d.nii',conName{n},i));
                newName = fullfile(baseDir, sprintf(glmDir, glm), sprintf('S%02d', sn), sprintf('%s_%s.nii',conName{n},SPM.xCon(i).name));
...
%        load('SPM_info.mat');
    ...
    case 'SURF:freesurfer'              % SURFACE PREPROCESS 1: Run recon-all on freesurfer
        ...
        freesurfer_reconall(freesurf_dir,sprintf('S%02d',sn),fullfile(baseDir,anatomicalDir,sprintf('S%02d',sn),[sprintf('S%02d',sn) '_anatomical.nii']));  
    case 'WB:surf_resample' % reslice indiv surfaces into fs_lr standard mesh
        % sn = subj_vec; cwd = pwd;
        surf  = '32k'; % 164k or 32k vertices
        vararginoptions(varargin, {'sn'});
        % for s = sn
        fprintf('reslicing S%02d\n', sn);
        freesurferDir = fullfile(baseDir, freesurferDir);
        wbDir = fullfile(baseDir, wbDir);
        surf_resliceFS2WB(sprintf('S%02d', sn), freesurferDir, wbDir, 'resolution', surf);
        % end
    ...
    case 'WB:vol2surf_indiv' % map indiv vol contrasts (.nii) onto surface (.gifti)
            ...
            for h = hemis
                surfDir = fullfile(baseDir, wbDir, sprintf('S%02d', sn));
                white = fullfile(surfDir, sprintf('%s.%s.white.%s.surf.gii', sprintf('S%02d', sn), hem{h}, surf));
                pial = fullfile(surfDir, sprintf('%s.%s.pial.%s.surf.gii', sprintf('S%02d', sn), hem{h}, surf));
                C1 = gifti(white);
                C2 = gifti(pial);
                subjGLM = fullfile(baseDir,sprintf(glmDir, glm), sprintf('S%02d', sn));
                ...
                outfile = fullfile(surfDir, sprintf('%s.%s.glm%d.%s.func.gii', sprintf('S%02d', sn), hem{h}, glm, map));
...
%         dircheck(groupDir);
        ...
        for h = hemis
            ...
            columnName = {};
            for s = sn
                inputFiles{end+1} = fullfile(baseDir, wbDir, sprintf('S%02d', s), sprintf('%s.%s.glm%d.%s.func.gii', sprintf('S%02d', s), hem{h}, glm, map));
            ...
            switch map
                case 't' % t-values maps (univariate GLM)
                    ...
                    nc  = numel(con);
                    for ic = 1:nc
                        groupfiles{ic}  = fullfile(groupDir, sprintf('group.%s.%s.glm%d.%s.func.gii', map, hem{h}, glm, con(ic).name));
                    end
                case 'con' % contrast beta maps (univariate GLM)
                    con = SPM.xCon;
                    for ic = 1:numel(con)
...
%         cwd = pwd;
        ...
        for s = sn
            ...
            cd(fullfile(baseDir,sprintf(glmDir, glm), sprintf('S%02d', s))); % cd to subject's GLM dir
            load SPM;
...
%             'leg',regname{roi},'leglocation','bestoutside'); % ,
    ...
    case 'ROI:redefine' % use this...
        ...
        for h=hemis
            file=fullfile(baseDir,sprintf(glmDir,glm),sprintf('S%02d',sn),'mask.nii');
            pathtosurf=fullfile(atlasDir,sprintf('FS_LR_%s',surf));
            surface=fullfile(pathtosurf,sprintf('fs_LR.%sk.%s.flat.surf.gii',surf,hem{h}));
            for r=1:n_roi
                idx=idx+1;
                R{idx}.type     = 'surf_nodes';
                R{idx}.white    = fullfile(surfDir,sprintf('%s.%s.white.%sk.surf.gii',sprintf('S%02d',sn),hem{h},surf));
                R{idx}.pial     = fullfile(surfDir,sprintf('%s.%s.pial.%sk.surf.gii',sprintf('S%02d',sn),hem{h},surf));
                R{idx}.flat     = surface;
                R{idx}.linedef  = [5,0,1];                             % take 5 steps along node between white (0) and pial (1) surfaces
                R{idx}.image    = file;
                R{idx}.name     = [sprintf('S%02d',sn) '_' ROI_name{r+1} '_' hem{h}];
...
%        sn=subj_vec;
        ...
        % load
        loadname=fullfile(baseDir,roiDir,sprintf('psc_glm%d_%s_N=%d.mat',glm,'SSS',numel(sn)));
        T=load(loadname);
        ...
        CAT.edgecolor = 'none';
                  ...
                  % 1     2     3    4    5     6      7      8     9
        ROI_name={'SMA','PMv','PMd','M1','S1','SPLa','SPLp','DSVC','MT+','VSVC','EAC'};
        %ROI_name={'SMA','PMv','PMd','M1','S1','SPLa','SPLp','MT+','VSVC','EAC'};
...
%         save(fullfile(pathToSave,'ROI_MotorvsCue.png'));
        if ptype==1
            legend_text = {'Motor','Cue'};
            title_text = 'Repetition effects';
            x_coord = barplot([T.roi],T.normpsc,'split',[T.cond],'CAT',CAT,'gapwidth',[0.8 0. 0 0],'barwidth',1,...
                'leg',legend_text,'subset',T.cond<3);
        elseif ptype==2
            legend_text = {'Letter','Spatial'};
            title_text = 'Repetition effects (Motor)';
            x_coord = barplot([T.roi],T.normpsc,'split',[T.cond],'CAT',CAT,'gapwidth',[0.8 0. 0 0],'barwidth',1,...
